...
export function genFloorFrame(spec: VanSpec): Part[] {
  ...
  const parts: Part[] = []
  const timber = (id: string, label: string, pos: Vec3, size: Size, axis: Axis, layer: LayerId = 'floorFrame', approx = false) =>
    parts.push(box(id, layer, label, 'timber', pos, size, { axis, cut: { cutLength: size.l, profile, fromStock: true }, approximate: approx || undefined }))
  ...
  ;[{ x: 0, tag: 'P' }, { x: W - bw, tag: 'D' }].forEach(({ x, tag }) => {
    if (z0 > 0) timber(`rail-wall${tag}-front`, `Wall rail (${tag}) front`, { x, y: 0, z: 0 }, { l: z0, w: bw, h: bh }, 'z', 'floorFrame', true)
    if (length > z1) timber(`rail-wall${tag}-rear`, `Wall rail (${tag}) rear`, { x, y: 0, z: z1 }, { l: length - z1, w: bw, h: bh }, 'z', 'floorFrame', true)
  })
  ...
  ;[{ x0: 0, tag: 'P', side: 'passenger' }, { x0: W - inset, tag: 'D', side: 'driver' }].forEach((a) => {
    const y = archH
    timber(`archbox-${a.tag}-railOut`, `Arch box rail (${a.side} outer)`, { x: a.x0, y, z: z0 }, { l: archLen, w: bw, h: bh }, 'z', 'archBoxes', true)
    timber(`archbox-${a.tag}-railIn`, `Arch box rail (${a.side} inner)`, { x: a.x0 + inset - bw, y, z: z0 }, { l: archLen, w: bw, h: bh }, 'z', 'archBoxes', true)
    // corner posts down to the floor
    ;[[a.x0, z0], [a.x0 + inset - bw, z0], [a.x0, z1 - bw], [a.x0 + inset - bw, z1 - bw]].forEach((c, ci) =>
      timber(`archbox-${a.tag}-post-${ci}`, `Arch box post (${a.side})`, { x: c[0], y: 0, z: c[1] }, { l: archH, w: bw, h: bh }, 'y', 'archBoxes', true),
    )
